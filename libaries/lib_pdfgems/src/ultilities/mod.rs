...
#[allow(deprecated)]
pub fn read_base64(base64: &str) -> Result<Document, Box<dyn Error>> {
    let decoded_bytes = base64::decode(base64)?;
    let mut byte_slice: &[u8] = &decoded_bytes;

    //let mut doc = Document::extract_stream(&self, stream_id, decompress);
    //let mut doc = Document::decode_text(encoding, bytes);
    let doc = Document::load_mem(byte_slice)?; // Load a PDF document from a memory slice.
    Ok(doc)
}
...
pub fn merge_pdf(documents: Vec<Document>) -> Result<Document, PdfError> {
