...
use crate::models::MediaDownloadResponse;
...
pub async fn get_image_upload_link(
    ...
    let link = utilities::get_input_link(&client, &key, &bucket).await?;
...
pub async fn get_image_download_link(
    ...
    key.push_str(&file_name);
    let bucket = env::var("AWS_BUCKET").map_err(|e| Errors::Error(e.to_string()))?;
    let link = utilities::get_output_link(&client, &key, &bucket).await?;
